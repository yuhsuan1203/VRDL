        ...
        for i, data in pbar:
            ...
            if training:
                if self.hard_mining and i % 10 == 0 and self.cache is not None:
                    ...
                    self.cache = None
                    self.cached_loss = 0
                pbar.set_postfix(**{k: "{:.5f}".format(v.cpu().numpy() / (i + 1)) for k, v in avg_meter.items()})
        return {k: v.cpu().numpy() / len(loader) for k, v in avg_meter.items()}   #[0][0]
...
def train_config(config_path):
    print("Now using config:   " + config_path)
    config = get_config(config_path, predict=False)
