...
def biggest_contour(im):
    contours, hierarchy = cv2.findContours(np.copy(im), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def load_from_files(test_image_paths, output_dir=None, is_quiet=False):
    ...
    img_idx = list(map(os.path.basename, all_img_filenames))
...
def average_from_files(test_image_paths, probs_dirs, output_dir, should_save_masks=True,
    ...
    #print("Here is probs_dirs" + str(probs_dirs))
    all_rles = []
    all_img_filenames = []
...
def main():
    ...
    parser.add_argument('-j', '--n_jobs', type=int, default=4, metavar='N',
    ...
    probs_dirs = list(map(lambda x: (Path(join(config.submissions_dir, x[0])), float(x[1]) / w_sum), probs_dirs))
    print('Weights:{}'.format([x[1] for x in probs_dirs]))
    output_dir = Path(config.submissions_dir) / ('fourth_submission')
    ...
    print(len(test_pathes))
    print('Reading from' + str(list(map(str, probs_dirs))))
